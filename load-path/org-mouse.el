...
;; Version: 0.25
;; $Id: org-mouse.el 1086 2007-05-22 23:59:32Z pz215 $
;; 
...
(defun org-mouse-line-position ()
  "Returns :beginning or :middle or :end, depending on the point position in the line.

If the point is at the end of the line, return :end.
If the point is separated from the beginning of the line only by white space and *'s (`org-mouse-bolp'), return :beginning.  Otherwise, return :middle."
  (cond
   ((eolp) :end)
...
(defun org-mouse-remove-match-and-spaces ()
  "Remove the matched region and ensure that there is exactly one space around the point."
  (interactive)
  (replace-match "")
  (just-one-space))
...
(defun org-mouse-show-headlines ()
  "Change the visibility of the current org buffer so that only headlines are shown."
  (interactive) 
...
(defun org-mouse-show-overview ()
  "Change the visibility of the current org buffer so that only first-level headlines are shown."
  (interactive) 
...
(defun org-mouse-get-priority (&optional default)
  "Return the priority of the current headline or DEFAULT if no priority has been assigned."
  (save-excursion 
