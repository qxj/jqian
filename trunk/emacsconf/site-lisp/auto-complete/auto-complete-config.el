...
(unless (fboundp 'semantic-ia-get-completions)
  (defun semantic-ia-get-completions (context point)
    "Fetch the completion of CONTEXT at POINT. Supports caching."
    (let ((symbols
...
(defun ac-semantic-candidate (prefix)
  (if (memq major-mode ac-semantic-modes)
      (mapcar 'semantic-tag-name
              (ignore-errors
                (or (senator-find-tag-for-completion (regexp-quote prefix))
                    (semantic-ia-get-completions
